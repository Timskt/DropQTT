...
- 🔀 **Broker-to-Broker Data Bridge**: Run two independent bridge connections and forward messages verbatim between any two brokers by topic rules — wildcard source filters (`+`/`#`), keep-topic or prefix-remap targets, follow/fixed QoS and retain policies, MQTT5 property forwarding, per-rule hit counters, a live forward log, and auto-reconnect that restores remembered endpoints on startup.
